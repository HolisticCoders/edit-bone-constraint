...
def instanciate_constraint_from_property(prop, bone):
    if prop.type in _all_constraints:
        return _all_constraints[prop.type](prop, bone)
    else:
        raise TypeError(
            "No class named corresponding to constraint type {}".format(prop.type)
        )
